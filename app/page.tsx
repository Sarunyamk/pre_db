...
export default function HomePage() {
  ...
  const handleNextStep = () => {
    ...
    if (step === 1) {
      if (
        !formData.prefix ||
        !['Mr', 'Ms', 'Mrs', 'Other'].includes(formData.prefix)
      ) {
        currentErrors.push({
          field: 'prefix',
          message: 'Prefix is required and must be valid.',
        })
      }
      if (!formData.firstName || !/^[a-zA-Zก-๙\s]+$/.test(formData.firstName)) {
        currentErrors.push({
          field: 'firstName',
          message:
            'First Name only contain characters (English or Thai), and no special symbols.',
        })
      }
      if (!formData.lastName || !/^[a-zA-Zก-๙\s]+$/.test(formData.lastName)) {
        currentErrors.push({
          field: 'lastName',
          message:
            'Last Name must only contain characters (English or Thai), and no special symbols',
        })
      }
    ...
    if (step === 3) {
      if (
        ...
        ![
          ...
          'Doctorate',
          'Other',
        ].includes(formData.education)
      ) {
        currentErrors.push({
          field: 'education',
          message: 'Education is required and must be valid.',
        })
      }
  ...
  const handleResetForm = () => {
    try {
      ...
      toast.error(`Error resetting form: ${error}`)
    }
  }
              ...
              <button
                ...
                className={`px-4 py-2 rounded text-white ${
                  step === 1
                    ...
                    : 'bg-blue-500 hover:scale-105 duration-300'
                }`}
              >
                Previous
              </button>
